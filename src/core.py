...
def wavelet_denoise(
    data: np.ndarray, wavelet: str = "db6", noise_sigma: float = None
) -> np.ndarray:
...
def preprocess_data(
    df: pd.DataFrame, features: List[str], wavelet: str = "db6"
    ...
    df_train_scaled = pd.DataFrame(
        scaler.fit_transform(df_train[features]), columns=features, index=df_train.index
    )
...
def create_dataset(
    X: pd.DataFrame, time_steps: int = 100, step: int = 10
) -> np.ndarray:
...
def reshape_for_tensor(
    X: np.ndarray, n_cols: int, target_shape: Tuple[int, int] = (10, 10)
) -> np.ndarray:
