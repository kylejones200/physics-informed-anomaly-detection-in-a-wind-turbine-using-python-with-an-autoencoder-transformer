...
def wavelet_denoise(data: np.ndarray, wavelet: str='db6', noise_sigma: float | None=None) -> np.ndarray:
...
def preprocess_data(df: pd.DataFrame, features: List[str], wavelet: str='db6') -> Tuple[pd.DataFrame, StandardScaler]:
...
def detect_anomalies(df_train: pd.DataFrame, contamination: float=0.01, n_estimators: int=100, random_state: int=42) -> Tuple[np.ndarray, pd.DataFrame]:
...
def plot_anomaly_detection(df: pd.DataFrame, anomalies: pd.DataFrame, feature: str, output_path: Path, plot: bool=False):
    ...
    ax.scatter(normal_data.index, normal_data[feature], c='#4A90A4', label='Normal', s=20, alpha=0.6)
    ax.scatter(anomalies.index, anomalies[feature], c='#D4A574', label='Anomaly', s=30, alpha=0.8)
    ax.set_xlabel('Time')
...
def plot_correlation_heatmap(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    if plot:
        ...
        sns.heatmap(corr, annot=True, mask=mask, cmap='coolwarm', center=0, square=True, linewidths=0.5, cbar_kws={'shrink': 0.8}, ax=ax)
        plt.savefig(output_path, dpi=100, bbox_inches='tight')
        plt.close()
